...
class FreeFallAnalyzer:
    ...
    def load_data(self):
        ...
        try:
            ...
            # 安全な数値抽出でソート（幅オーバーなどで '*' 混入ファイルは除外）
            files = glob.glob(state_pattern)
            pattern = re.compile(r'state_step_(\d+)\.csv$')
            file_steps = []
            for f in files:
                m = pattern.search(os.path.basename(f))
                if m:
                    file_steps.append((int(m.group(1)), f))
            self.state_files = [f for _, f in sorted(file_steps)]
    ...
    def extract_trajectory_data(self) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        for file_path in self.state_files:
            try:
                # まずカンマ区切りCSVとして読み込みを試行
                try:
                    df = pd.read_csv(file_path)
                except:
                    # カンマ区切り失敗の場合、空白区切りとして読み込み
                    df = pd.read_csv(file_path, sep=r'\s+', engine='python')
                
                assert isinstance(df, pd.DataFrame), f"ファイル {file_path} の読み込みに失敗"
                if len(df) > 0:
                    # Particle_ID列があるかチェック
                    if 'Particle_ID' in df.columns:
                        # 粒子1のデータを抽出（複数粒子対応）
                        particle_1_data = df[df['Particle_ID'] == 1]
                        if len(particle_1_data) > 0:
                            times.append(float(particle_1_data['Time'].iloc[0]))  # type: ignore
                            x_positions.append(float(particle_1_data['X'].iloc[0]))  # type: ignore
                            z_positions.append(float(particle_1_data['Z'].iloc[0]))  # type: ignore
                            z_velocities.append(float(particle_1_data['Vz'].iloc[0]))  # type: ignore
                    else:
                        # 従来の単一粒子形式
